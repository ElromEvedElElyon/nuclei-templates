...
- **Capybara AI Swarm Mode**: ~/capybara-ai/swarm_mode.py — 5 modes (REPOUSO/ALERTA/OPERACAO/ENXAME/ANGEL)
  - Angel Protocol: 5 threads, maximum autonomous operation, 5/5 tasks completed
  - Loads 14 agent JSONs from ~/israel-ten/agents/, calls Gemini 2.5 Flash
  - `python3 ~/capybara-ai/swarm_mode.py [status|angel|enxame|operacao|alerta|run]`
- **14 Agent JSON Definitions**: ~/israel-ten/agents/*.agent.json
  - bounty-hunter, code-reviewer, content-creator, evolution-engine, lion
  - market-watcher, orchestrator, publisher, revenue-tracker, security-scanner
  - sentinel, social-commander, stability-guardian, translator
  - Each has: tools, mcp_tools, capabilities, skills (with steps), prompt_template, config
  - Loader: ~/israel-ten/agents/agent_loader.py
- **Bible Translation Pipeline**: ~/biblia-sagrada/translate_bible.py
  - Uses Gemini 2.5 Flash (FREE), 21 languages, chapter-by-chapter, resumable
  - EN translation running: 514/31105 verses (Genesis 20/50 chapters)

## COMPETITIVE LANDSCAPE (Session 80 Research)
- **VERSES AXIOM**: ONLY other framework with math foundation (FEP/Active Inference). 60% better than DreamerV3, 99% less compute
- **GPT-5.4** (5 Mar 2026): 5 variants, 1M ctx, 75% OSWorld, $2.50/$15 MTok
- **Claude Opus 4.6**: 1M ctx GA, 74%+ SWE-bench, $5/$25 MTok
- **Gemini 3.1 Pro**: 94.3% GPQA Diamond (BEST), 80.6% SWE-bench, $2/$12 MTok
- **Grok 4.20**: 2M context (LARGEST)
- **Llama 4 Maverick**: 128-expert MoE, FREE open-weight, beats GPT-4o
- **CrewAI**: 100K+ devs, $0-$120K/yr | **LangGraph**: Klarna/LinkedIn/Uber
- **Codex**: 2M+ weekly users, cloud agents + plugins (Mar 2026)
- **Cursor**: 8 parallel agents + Cloud Agents (Feb 2026)
- **NO framework generates revenue natively** except ElizaOS (x402 crypto)
- **Our position**: #7/10 globally. Unique: RSD math framework (only us + AXIOM have math)
